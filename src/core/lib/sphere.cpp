...
std::unique_ptr<Sphere> Raytracer::create_sphere_instance(const Point& center, double radius, const Color& color) {
    auto create_sphere = sphereLibraryLoader.getFunction("create_sphere");
    return std::unique_ptr<Sphere>(create_sphere(center, radius, color));
}
